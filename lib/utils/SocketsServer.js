...
class SocketsServer {
  ...
  start(server) {
    if (this.config.sockets.port) {
      this.logger.info(`Starting sockets server on port ${this.config.sockets.port}`);
      this.io = socketIO(this.config.sockets.port, {
        path: this.config.sockets.path,
      });
    } else {
      this.logger.info(`Starting sockets server on port ${this.config.server.port}`);
  ...
  onClientJoin(socket, room, cb) {
    if (this.config.sockets.rooms.indexOf(room) !== -1) {
      ...
      this.events.emit('socket-client-join', {
        socket,
        room,
      });
