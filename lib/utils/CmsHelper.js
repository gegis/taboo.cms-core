...
class CmsHelper {
  ...
  getRouteInitialSetup(route) {
    return async (ctx, next) => {
      if (ctx.route) {
        ...
        ctx.routeParams.aclResource = !!(route.options && route.options.aclResource);
      }
      return next();
    };
  }
  ...
  async onBeforeTemplateRender(ctx, tpl, params) {
    if (this.app.beforeTemplateRender && this.app.beforeTemplateRender.length > 0) {
      let promises = this.app.beforeTemplateRender.map(fn => {
        fn(ctx, tpl, params);
      });
