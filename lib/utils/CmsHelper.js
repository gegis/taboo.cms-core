...
class CmsHelper {
  ...
  getModuleSetup(moduleName) {
    ...
    const moduleControllers = this.filesHelper.getAllFileNames(path.join(modulePath, modulesConfig.controllersDir));
    const moduleModels = this.filesHelper.getAllFileNames(path.join(modulePath, modulesConfig.modelsDir));
    const moduleServices = this.filesHelper.getAllFileNames(path.join(modulePath, modulesConfig.servicesDir));
    const moduleHelpers = this.filesHelper.getAllFileNames(path.join(modulePath, modulesConfig.helpersDir));
    try {
      // Order is important
      if (this.filesHelper.fileExists(moduleConfigPath)) {
        ...
        if (module.config.installed && module.config.enabled) {
          ...
          this.setupModelConfigs(module, moduleModels);
  ...
  setupModelConfigs(module, moduleModels) {
    if (moduleModels) {
      [...moduleModels].map(name => {
        const prettyName = name.replace('Model.', '.').replace('.js', '');
        module.modelConfigs[prettyName] = require(path.join(module.path, this.config.server.modules.modelsDir, name));
      });
    }
  }
  ...
  setupServices(module, moduleServices) {
    if (moduleServices) {
      [...moduleServices].map(name => {
        ...
        module.services[prettyName] = require(path.join(module.path, this.config.server.modules.servicesDir, name));
      });
    }
  }
  ...
  setupHelpers(module, moduleHelpers) {
    if (moduleHelpers) {
      [...moduleHelpers].map(name => {
        ...
        module.helpers[prettyName] = require(path.join(module.path, this.config.server.modules.helpersDir, name));
      });
    }
  }
  ...
  composeResponse(ctx, layoutTpl, pageTpl, params) {
    ...
    tplConfig._body = ejs.render(pageTpl, tplConfig, this.config.server.ejsOptions);
    return ejs.render(layoutTpl, tplConfig, this.config.server.ejsOptions);
  }
  ...
  composeTemplate(ctx, tpl, params) {
    ...
    return ejs.render(tpl, tplConfig, this.config.server.ejsOptions);
  }
