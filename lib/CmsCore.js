...
class CmsCore {
  ...
  async setupAppModules() {
    ...
    let allRouteMethods = [];
    let allRoutes = [];
    let moduleName, module, methodAndPath, promises;
  ...
  async setupModels() {
    ...
    let moduleName, module, modelName, modelConfig, promises;
    for (moduleName in modules) {
      module = modules[moduleName];
      if (!module.models) {
        module.models = {};
    ...
    promises = afterModelsSetup.map(async fn => {
      await fn(modules, this.app.aclResources);
    });
    await Promise.all(promises);
  }
