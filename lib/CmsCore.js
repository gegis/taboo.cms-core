...
class CmsCore {
  constructor() {
    ...
    this.sockets = new SocketsServer(config); //sockets server
    this.koaApp = new Koa();
  ...
  //TODO remove this
  // setupUtils() {
  //   // this.app.logger.setup(this.app.config);
  //   // EventsEmitter.setup(this.app);
  //   // this.cmsHelper.setup(this.app);
  ...
  setupAppModules() {
    ...
    allRoutes = this.arrayHelper.sortByProperty(allRoutes, 'order');
    ...
    for (name in connections) {
      if (connections.hasOwnProperty(name)) {
        ...
        if (config.adapter) {
          ...
          this.logger.info(
            `Successfully established '${name}' connection: ${this.app.dbConnections[name].connectedTo}`
          );
        }
      }
    }
  ...
  startServer() {
    return new Promise(resolve => {
      this.server = http.createServer(this.koaApp.callback()).listen(this.config.server.port, () => {
  ...
  //   // TODO ????? do we need call taboo. here?
  //   if (this.app.dbConnections[connectionName]) {
  //     return this.app.dbConnections[connectionName];
  //   }
  //   return null;
  // }
  ...
  startSocketsServer() {
    if (this.config.sockets.enabled) {
      this.sockets.start(this.server);
    }
  }
}
...
module.exports = {
  ...
  // locales: cmsCore.app.locales,
  // adminLocales: cmsCore.app.adminLocales,
  // dbConnections: cmsCore.app.dbConnections,
  // Model: moduleModel => {
  ...
  //   return this.app.apiHelper.parseRequestParams(requestParams, paramsList);
  // },
  // cleanTimestamps: data => {
  //   this.app.apiHelper.cleanTimestamps(data);
  // },
